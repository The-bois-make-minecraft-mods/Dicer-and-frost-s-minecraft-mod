...
package net.mcreator.frostanddicersvanillaenhanced.world.dimension;
...
import net.mcreator.frostanddicersvanillaenhanced.item.ShadowdimensionItem;
import net.mcreator.frostanddicersvanillaenhanced.block.ShadowstoneblockBlock;
import net.mcreator.frostanddicersvanillaenhanced.block.PurplegoopBlock;
import net.mcreator.frostanddicersvanillaenhanced.block.DeathblockBlock;
...
public class ShadowdimensionDimension extends FrostAndDicersVanillaenhancedModElements.ModElement {
	@ObjectHolder("frost_and_dicers_vanillaenhanced:shadowdimension")
	public static final ModDimension dimension = null;
	@ObjectHolder("frost_and_dicers_vanillaenhanced:shadowdimension_portal")
	public static final CustomPortalBlock portal = null;
	public static DimensionType type = null;
	private static Biome[] dimensionBiomes;
	public ShadowdimensionDimension(FrostAndDicersVanillaenhancedModElements instance) {
		super(instance, 68);
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("shadowdimension"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("frost_and_dicers_vanillaenhanced:shadowdimension")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("frost_and_dicers_vanillaenhanced:shadowdimension"), dimension, null, false);
		}
		type = DimensionType.byName(new ResourceLocation("frost_and_dicers_vanillaenhanced:shadowdimension"));
	}
	...
	public void initElements() {
		...
		elements.items.add(() -> new ShadowdimensionItem().setRegistryName("shadowdimension"));
	}
	...
	public static class CustomPortalBlock extends NetherPortalBlock {
		public CustomPortalBlock() {
			...
			setRegistryName("shadowdimension_portal");
		}
		...
		private TeleporterDimensionMod getTeleporterForDimension(Entity entity, BlockPos pos, ServerWorld nextWorld) {
			BlockPattern.PatternHelper bph = ShadowdimensionDimension.CustomPortalBlock.createPatternHelper(entity.world, new BlockPos(pos));
			...
			protected int getDistanceUntilEdge(BlockPos pos, Direction directionIn) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.func_196900_a(this.world.getBlockState(blockpos))
							|| !(this.world.getBlockState(blockpos.down()).getBlock() == DeathblockBlock.block.getDefaultState().getBlock())) {
						break;
					}
				}
				BlockPos framePos = pos.offset(directionIn, i);
				return (this.world.getBlockState(framePos).getBlock() == DeathblockBlock.block.getDefaultState().getBlock()) ? i : 0;
			}
				...
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (!(this.world.getBlockState(framePos).getBlock() == DeathblockBlock.block.getDefaultState().getBlock())) {
								break label56;
							}
	...
	public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
		try {
			...
			poi = (PointOfInterestType) method.invoke(null, "shadowdimension_portal",
	...
	public static class TeleporterDimensionMod extends Teleporter {
					...
					for (int i8 = 1; i8 < 3; ++i8) {
						for (int i9 = -1; i9 < 3; ++i9) {
							...
							this.world.setBlockState(blockpos$mutable,
									flag ? DeathblockBlock.block.getDefaultState().getBlock().getDefaultState() : Blocks.AIR.getDefaultState());
						}
					}
				}
			}
			for (int k7 = -1; k7 < 3; ++k7) {
				for (int j8 = -1; j8 < 4; ++j8) {
					if (k7 == -1 || k7 == 2 || j8 == -1 || j8 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, DeathblockBlock.block.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
		...
		public BlockPattern.PortalInfo placeInExistingPortal(BlockPos p_222272_1_, Vec3d p_222272_2_, Direction directionIn, double p_222272_4_,
			...
			return optional.map((p_226707_7_) -> {
				...
				BlockPattern.PatternHelper blockpattern$patternhelper = ShadowdimensionDimension.CustomPortalBlock.createPatternHelper(this.world,
						blockpos);
	...
	public static class CustomDimension extends Dimension {
		...
		public Vec3d getFogColor(float cangle, float ticks) {
			return new Vec3d(0.207843137255, 0.113725490196, 0.360784313725);
		}
		...
		public ChunkProviderModded(IWorld world, BiomeProvider provider) {
			super(world, provider, new OverworldGenSettings() {
				public BlockState getDefaultBlock() {
					return ShadowstoneblockBlock.block.getDefaultState();
				}

				public BlockState getDefaultFluid() {
					return PurplegoopBlock.block.getDefaultState();
				}
			});
		...
		public BiomeProviderCustom(World world) {
			...
			for (Biome biome : this.biomes) {
				biome.addCarver(GenerationStage.Carving.AIR, Biome.createCarver(new CaveWorldCarver(ProbabilityConfig::deserialize, 256) {
					{
						carvableBlocks = ImmutableSet.of(ShadowstoneblockBlock.block.getDefaultState().getBlock(),
