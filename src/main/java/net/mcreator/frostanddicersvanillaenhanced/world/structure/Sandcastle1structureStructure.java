...
public class Sandcastle1structureStructure extends FrostAndDicersVanillaenhancedModElements.ModElement {
	public Sandcastle1structureStructure(FrostAndDicersVanillaenhancedModElements instance) {
		super(instance, 27);
	}
		...
		Feature<NoFeatureConfig> feature = new Feature<NoFeatureConfig>(NoFeatureConfig::deserialize) {
			...
			public boolean place(IWorld iworld, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
				...
				if (!dimensionCriteria)
					return false;
				if ((random.nextInt(1000000) + 1) <= 16000) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("frost_and_dicers_vanillaenhanced", "sandcastle1structure"));
						if (template == null)
							return false;
