...
package net.mcreator.frostanddicersvanillaenhanced.world.structure;
...
public class Sandcastle2structureStructure extends FrostAndDicersVanillaenhancedModElements.ModElement {
	public Sandcastle2structureStructure(FrostAndDicersVanillaenhancedModElements instance) {
		super(instance, 28);
	}
	...
	public void init(FMLCommonSetupEvent event) {
		Feature<NoFeatureConfig> feature = new Feature<NoFeatureConfig>(NoFeatureConfig::deserialize) {
			...
			public boolean place(IWorld iworld, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
				...
				if (!dimensionCriteria)
					return false;
				if ((random.nextInt(1000000) + 1) <= 17000) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("frost_and_dicers_vanillaenhanced", "sandcastle2structure"));
						if (template == null)
							return false;
		...
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			if (ForgeRegistries.BIOMES.getKey(biome).equals(new ResourceLocation("frost_and_dicers_vanillaenhanced:beachgetayways")))
				biomeCriteria = true;
			if (!biomeCriteria)
				continue;
