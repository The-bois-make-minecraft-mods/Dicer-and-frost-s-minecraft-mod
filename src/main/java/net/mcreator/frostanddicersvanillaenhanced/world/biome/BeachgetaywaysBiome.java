...
import net.mcreator.frostanddicersvanillaenhanced.block.CoconutplantBlock;
import net.mcreator.frostanddicersvanillaenhanced.block.BeachwoodlogBlock;
import net.mcreator.frostanddicersvanillaenhanced.block.BeachwoodleavesBlock;
...
public class BeachgetaywaysBiome extends FrostAndDicersVanillaenhancedModElements.ModElement {
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, new CustomTreeFeature()
					.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(BeachwoodlogBlock.block.getDefaultState()),
							new SimpleBlockStateProvider(BeachwoodleavesBlock.block.getDefaultState()))).baseHeight(7)
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == Blocks.SAND.getDefaultState().getBlock() || ground == Blocks.SANDSTONE.getDefaultState().getBlock())
							&& (ground2 == Blocks.SAND.getDefaultState().getBlock() || ground2 == Blocks.SANDSTONE.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), Blocks.SANDSTONE.getDefaultState(), bbox);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == BeachwoodleavesBlock.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, BeachwoodleavesBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, BeachwoodlogBlock.block.getDefaultState(), bbox);
								...
								for (Direction Direction : Direction.Plane.HORIZONTAL) {
									if (rand.nextInt(4 - hlevel) == 0) {
										...
										setTreeBlockState(changedBlocks, world, position.add(dir.getXOffset(), height - 5 + hlevel, dir.getZOffset()),
												CoconutplantBlock.block.getDefaultState(), bbox);
									}
								}
							}
						}
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == BeachwoodlogBlock.block.getDefaultState().getBlock()
					|| blockType == BeachwoodleavesBlock.block.getDefaultState().getBlock() || blockType == Blocks.SAND.getDefaultState().getBlock()
					|| blockType == Blocks.SANDSTONE.getDefaultState().getBlock();
		}
