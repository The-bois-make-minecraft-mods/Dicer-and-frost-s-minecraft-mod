...
import net.mcreator.frostanddicersvanillaenhanced.block.ShadowlogsBlock;
import net.mcreator.frostanddicersvanillaenhanced.block.ShadowleavesblockBlock;
import net.mcreator.frostanddicersvanillaenhanced.block.ShadowgrassBlock;
...
public class WhisperingforestBiome extends FrostAndDicersVanillaenhancedModElements.ModElement {
	@ObjectHolder("frost_and_dicers_vanillaenhanced:whisperingforest")
	public static final CustomBiome biome = null;
	public WhisperingforestBiome(FrostAndDicersVanillaenhancedModElements instance) {
		super(instance, 70);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			...
			setRegistryName("whisperingforest");
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == ShadowgrassBlock.block.getDefaultState().getBlock()
							|| ground == ShadowstoneblockBlock.block.getDefaultState().getBlock())
							&& (ground2 == ShadowgrassBlock.block.getDefaultState().getBlock()
									|| ground2 == ShadowstoneblockBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), ShadowstoneblockBlock.block.getDefaultState(), bbox);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == ShadowleavesblockBlock.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, ShadowleavesblockBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, ShadowlogsBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == ShadowlogsBlock.block.getDefaultState().getBlock()
					|| blockType == ShadowleavesblockBlock.block.getDefaultState().getBlock()
					|| blockType == ShadowgrassBlock.block.getDefaultState().getBlock()
					|| blockType == ShadowstoneblockBlock.block.getDefaultState().getBlock();
		}
